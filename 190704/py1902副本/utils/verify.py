...
FONT_PATH = os.path.join(settings.BASE_DIR, 'static', 'font', 'Yahei.ttf')
# 显示几个验证码
LEN_VERIFY = 4
LEN_VERIFY = 4
...
def get_verify():
    ...
    for t in range(verify_len):
        ...
        verify += text
        # 向画布上绘制 验证码
        draw.text(((weight // verify_len) * t + 7, 10), text, font=font,fill=(random.randint(32, 127), random.randint(32, 127), random.randint(32, 127)))
        draw.text(((weight // verify_len) * t + 7, 10), text, font=font,fill=(random.randint(32, 127), random.randint(32, 127), random.randint(32, 127)))
    # 实例化 字节io（输入 输出）对象
    img_buffer = BytesIO()
    img_buffer = BytesIO()
